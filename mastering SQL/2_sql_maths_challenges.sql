...
SELECT * FROM Invoice;
SELECT MAX(Total) - MIN(Total) AS price_difference FROM Invoice;

-- 2. What is the difference in length between the longest and shortest track in minutes?
SELECT * FROM Track;
SELECT MAX(Milliseconds) - MIN(Milliseconds) AS seconds_diff FROM Track;

-- 3. What is the average length of a track in the 'Rock' genre in minutes?
SELECT AVG(MAX(Milliseconds) - MIN(Milliseconds)) FROM Track;
